<template>
  <div v-bind="$attrs" class="quote-message-wrapper" :class="{ 'has-quote': hasReply }">
    <div v-for="(item, index) in processedMessage" :key="index" class="message-part-container">
      <pre v-if="item.type === 'normal' || item.type === 'reply' || item.type === 'mention'" :class="['message-part', item.type]">
        {{ item.content }}
      </pre>
      <img v-else-if="item.type === 'emoji'" :src="item.content" :class="['message-emoji', { 'big-emoji': item.isBigFace }]" :alt="item.alt" @error="handleEmojiError($event, index)" />
    </div>
  </div>
</template>
<script setup>
...
function processContentWithEmojis(content, result) {
  ...
  try {
    ...
    while ((match = mentionPattern.exec(content)) !== null) {
      ...
      const displayName = props.userInfoMap[mentionedUsername]?.nickname || mentionedUsername;
    ...
    while ((match = replyPattern.exec(content)) !== null) {
      ...
      while ((tempMatch = tempEmojiPattern.exec(innerReplyContent)) !== null) {
        ...
        if (tempMatch.index > tempLastIndex) {
          replyParts.push({
            type: "text",
            content: innerReplyContent.substring(tempLastIndex, tempMatch.index)
          });
        }
    ...
    if (lastIndex < content.length) {
      ...
      while ((emojiMatch = emojiPattern.exec(remainingContent)) !== null) {
        ...
        if (emojiMatch.index > emojiLastIndex) {
          result.push({
            type: "normal",
            content: remainingContent.substring(emojiLastIndex, emojiMatch.index)
          });
        }
